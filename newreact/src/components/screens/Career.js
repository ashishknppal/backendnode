...
function Taxcode() {
  ...
  const get_careerfun = async (page, search = "") => {
    try {
      const res = await axios.get(`${endpoint.CAREER}?page=${page}&search=${search}`);
  ...
  const add_cmsContent  = async () => {
    ...
    try {
      const res = await axios.post(`${endpoint.ADD_CAREER}`, payload, {
        headers: {
  ...
  const update_cmsContent = async () => {
    ...
    try {
      const res = await axios.put(`${endpoint.UPDATE_CAREER}/${selectedcontentid}`, payload, {
        headers: {
          "Content-Type": "application/json",
        },
      });
  ...
  const delete_career = async (item) => {
    try {
      const res = await axios.delete(`${endpoint.DELETE_CAREER}/${item.id}`);
  ...
  return (
    <div className="container-fluid product p-3 bg-white">
      <div className="card header">
        <h4 className="mb-3 mb-md-0">Career</h4>
