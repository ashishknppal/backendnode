...
function Client() {
  ...
  useEffect(() => {
    get_feedback(currentPage, searchTerm);
  }, [currentPage, searchTerm]);


  const get_feedback = async (page, search = "") => {
    setLoading(true);
    try {
      const res = await axios.get(`${endpoint.FEEDBACK}?page=${page}&search=${search}`, {
        headers: {
  ...
  const delete_feedback = async (Client) => {
    try {
      const res = await axios.delete(`${endpoint.DELETE_FEEDBACK}/${Client.id}`);
  ...
  return (
    <div className="container-fluid product p-3 bg-white">
      <div className="card header">
        <h4 className="mb-3 mb-md-0">Feedback</h4>
