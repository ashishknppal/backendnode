...
import ProtectedRoute from './../authenticate';
...
const Contract = () => {
  ...
  const [products, setProducts] = useState(() => {
    try {
        const storedProducts = JSON.parse(localStorage.getItem("invoiceProducts"));
  ...
  const add_contract = async () => {
    ...
    const data = {
      renewal_frequency:selectedFrequency.id,
      ...
      round_off: round_off,
      net_amount: calculateGrandTotal(),
      tax_amt: calculateTotalTax(),
      sub_total: calculateSubTotal(),
      products: products,
    };
    try {
      const res = await axios.post(`${endpoint.ADD_CONTRACT}`, data, {
        headers: {
          "Content-Type": "application/json", 
        },
      });
      ...
      console.error("add contract catch error:", error);
    }
  };
  const clearProducts = () => {
  ...
  return (
    <div className="container-fluid bg-white invoice p-2">
      ...
      <hr />
      <div className="card" style={{border: "1px solid #5a28ab"}}>
            ...
            <div className="row mb-3">
                ...
                <Select
                    ...
                    onChange={(values) => {
                    console.log("Selected Frequency:", values);
                    setSelectedFrequency(values[0] || null);
                    }}
                    multi={false}
                />
                </div>
      ...
      <hr />
      <button className="savebutton" onClick={add_contract}>
        Save
      </button>
    </div>
  );
