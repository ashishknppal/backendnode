...
function Taxcode() {
  ...
  const [getnews, set_news] = useState([]);
  const [title, career_title] = useState("");
  ...
  useEffect(() => {
    get_careerfun(currentPage, searchTerm);
  }, [currentPage, searchTerm]);


  const get_careerfun = async (page, search = "") => {
    try {
      const res = await axios.get(`${endpoint.NEWS}?page=${page}&search=${search}`);
    
      setTotalItems(res.data.totalItems);
      set_news(res.data.data);
    } catch (error) {
  ...
  const add_cmsContent  = async () => {
    ...
    try {
      const res = await axios.post(`${endpoint.ADD_NEWS}`, payload, {
        headers: {
  ...
  const validateInputs = () => {
    const exist_title = getnews.find(
  ...
  const update_cmsContent = async () => {
    ...
    try {
      const res = await axios.put(`${endpoint.UPDATE_NEWS}/${selectedcontentid}`, payload, {
        headers: {
          "Content-Type": "application/json",
        },
      });
  ...
  const delete_career = async (item) => {
    try {
      const res = await axios.delete(`${endpoint.DELETE_NEWS}/${item.id}`);
  ...
  return (
    <div className="container-fluid product p-3 bg-white">
      <div className="card header">
        <h4 className="mb-3 mb-md-0">News And Notification</h4>
            ...
            <div className="col-md-6">
              <div className="form-group">
                ...
                <input
                  ...
                  id="title"
                  value={title}
                  onChange={(e) => career_title(e.target.value)}
